...
/*	int		geo_flip;		// geometric effect applied, flipped */
	...
	Dvector	wirescan;		/* position of wirescan 45 degree wire positioner (micron) */
	...
	double	wirescan;		/* position of wire scan 45 degree wire positioner (micron) */
	...
	#endif
	double	wirebaseX;		/* position of PM500, useful when wirescan at 45 deg. is present (micron) */
...
herr_t cp_h5obj(hid_t oid,const char *objname,const H5O_info_t *objinfo,void *param);
int HDF5WriteROI(hid_t file_id, const char *dataName, void *vbuf, size_t xlo, size_t xhi, size_t ylo, size_t yhi, hid_t dataType, struct HDF5_Header *head);
...
int createNewData(hid_t file_id, const char *dataName, int rank, int *dims, int dataType);
...
herr_t writeReconPgmInFile(hid_t file_id);
herr_t writeImageCosmicInFile(hid_t file_id, int cosmicIn);
herr_t writeImageNormalizationsInFile(hid_t file_id, double norm_exponent, double norm_threshold, double norm_rescale);
herr_t writeDepthInFile(hid_t file_id, double depth);
