...
//	if (!checkFileTypeLine(line,"geometryFileN")) goto exitPoint;	/* break out if checkFileType is false */
	...
	if (itype=1)									/* old file type with "$tag value" pairs */
		...
		/*	1<<2 == 4 */
		if (!strFromTagBuf(buf,"Ndetectors",line,250))	{ geo->Ndetectors = strtol(line,NULL,10); n=n|1<<0; }	/* # of detectors */
		...
		if (!strFromTagBuf(buf,"wireKnife",line,250))	{ geo->wire.knife = strtol(line,NULL,10); n=n|1<<8; }
