...
__device__ double device_pixel_xyz_to_depth(point_xyz pixelPos,               /* end point of ray, an xyz location on the detector */
...
int start(char* infile, char* outfile, char* geofile, double depth_start, double depth_end, double resolution, int first_image, int last_image, int out_pixel_type, int wireEdge, char* normalization, int cudaRowNo, int videoCardNo);
void processAll( int file_num_start, int file_num_end, char* fn_base, char* fn_out_base, char* normalization, int cudaRowNo, int videoCardNo);
...
void depth_resolve(int i_start, int i_stop, int videoCardNo);
...
int main (int argc, const char *argv[]) {
	...
	int      videoCardNo = 0;           /* the video card program is running on */
	...
	while (1)
	{
		static struct option long_options[] =
		{
			...
			{"GPU number",          required_argument,      0, 'V'},
		...
		c = getopt_long (argc, (char * const *)argv, "i:o:g:s::e:r:v::f:l:n:p:w:m:t:d:D:F:R:V:@::h::", long_options, &option_index);
		...
		switch (c)
		{
         ...
         case 'V':
            videoCardNo = atol(optarg);
            if (videoCardNo == 0 || videoCardNo == 1)
            {
            }
            else
            {
               error("-V number must be either 0 or 1\n");
               exit(1);
               return 1;
            }
            break;
...
=======
				...
				printf("\n-V <#>,\t --videoCardNumber=<#>\t\tnumber of video card to use --GPU parameters");
...
>>>>>>> .r143
	...
	if (verbose > 0) {
      ...
      printf("\nUse the video card = '%d'",videoCardNo);
		printf("\n\n");
	}
	fflush(stdout);

	start(infile, outfile, geofile, depth_start, depth_end, resolution, first_image, last_image, out_pixel_type, wireEdge, normalization, cudaRowNo, videoCardNo);
...
int videoCardNo)              /* video car number */
{
...
*/
	...
	processAll(first_image, last_image, infile, outfile, normalization, cudaRowNo, videoCardNo);
...
char	*normalization,				/* optional tag for normalization */
int	cudaRowNo,						/* cuda row number */
int   videoCardNo)               /* video card number */
{
...
<<<<<<< .mine
	time_t systime;
	systime = time(NULL);
	printf("\n staring depth resolve at %s, number of rows %s \n ",ctime(&systime),rows); 
	    depth_resolve(cur_start_i, cur_stop_i);
=======
		depth_resolve(cur_start_i, cur_stop_i, videoCardNo);
>>>>>>> .r143
	systime = time(NULL);
	printf("\n depth resolve  ended at %s",ctime(&systime));
...
void depth_resolve(int i_start,        /* starting row of this stripe */
                   ...
                   int videoCardNo)     /* video card number */
{
   ...
   paraPassed.CKIX = calibration.wire.ki.x;
   paraPassed.CKIY = calibration.wire.ki.y;
   paraPassed.CKIZ = calibration.wire.ki.z;
   paraPassed.UPDEPTHS = user_preferences.depth_start;
   paraPassed.UPDEPTHR = user_preferences.depth_resolution;
   paraPassed.IMDEPTHSIXE = image_set.depth_resolved.size;
   paraPassed.CWIREDIAMETER = calibration.wire.diameter;
   ...
   // malloc the storage for image_depth_intensity and backedge1
   if ((image_depth_intensity = (double *)malloc(user_preferences.NoutputDepths*sizeof(double))) == 0) {fprintf(stderr,"malloc1 Fail \n"); return;}
   if ((backedge1 = (point_xyz *)malloc(nx*sizeof(point_xyz))) == 0) {fprintf(stderr,"malloc1 Fail \n"); return;}
   ...
   for (step = 0; step < (size_t)(pixel_values.size-1); step++)
      for (j = 0; j < (size_t)imaging_parameters.nROI_j; j++)
         for (i = 0; i <= (size_t)gpustop; i++)
            c[i+j*nx+step*nx*ny] = gsl_matrix_get((const gsl_matrix *)image_set.wire_scanned.v[step], i , j);

   for (j = 0; j < (size_t)imaging_parameters.nROI_j; j++)
      for (i = 0; i <= (size_t)gpustop; i++)
         intensity[i+j*nx] = gsl_matrix_get(intensity_map, (i+i_start), j);

   for (step=0; step < (size_t)(user_preferences.NoutputDepths); step++)
      for (j=0; j < (size_t)imaging_parameters.nROI_j; j++)
         for (i = 0; i <= (size_t)gpustop; i++)
            image_depth[i+j*nx+step*nx*ny] = 0;

   for(i = 0; i < user_preferences.NoutputDepths; i++)
   {
      image_depth_intensity[i]=0;
   }
   ...
   cudaMalloc((void **) &d_backedge1, nx*sizeof(point_xyz));
      ...
      for (i = i_start; i <= (size_t)i_stop; i++) {                           /* loop over selected part of i */
         ...
         for (j=0; j < (size_t)imaging_parameters.nROI_j; j++) {              /* loop over all of j, wire travels in the j direction for the orange detector */
            if (j == 0)
            {
               ...
               backedge1[i-i_start].x=back_edge.x;
               backedge1[i-i_start].y=back_edge.y;
               backedge1[i-i_start].z=back_edge.z;
            }
            else
            {
         ...
         c1[i-i_start+j*nx]= front_edge.x;
         c1[i-i_start+j*nx+nx*ny]= front_edge.y;
         c1[i-i_start+j*nx+2*nx*ny]= front_edge.z;
      }
   }
   ...
   cudaMemcpy(d_backedge1, backedge1, nx*sizeof(point_xyz), cudaMemcpyHostToDevice);

   point_xyz *d_gpuPointArray;
   cudaMalloc((void**)&d_gpuPointArray, pixel_values.size*sizeof(point_xyz));

   cudaMemcpy(d_gpuPointArray, image_set.wire_positions.v, pixel_values.size*sizeof(point_xyz), cudaMemcpyHostToDevice);
   ...
   for (i = 0; i <= (size_t)gpustop; i++) {
      for (j=0; j < (size_t)imaging_parameters.nROI_j; j++) {
         for (step=0; step < (size_t)(user_preferences.NoutputDepths); step++) {
            ...
            *d += image_depth[i+j*nx+step*nx*ny];
         }
      }
   }
   ...
   for(i = 0; i < user_preferences.NoutputDepths; i++)
   {
      image_set.depth_intensity.v[i]+=image_depth_intensity[i];
   }
