...
//	if (!checkFileTypeOLD(buf,p-buf,"depthSortedInfo")) goto exitPoint;	/* check the file type */
	...
	if (!strFromTagBuf(buf,"ws_detectorNumber",line,250))	{ *detector = atol(line); n=n|1<<5; }				/* detector number */

	if (!strFromTagBuf(buf,"ws_firstInputIndex",line,250))	{ *first_image = atol(line); }						/* index to first input image */
	if (!strFromTagBuf(buf,"ws_lastInputIndex",line,250))	{ *last_image = atol(line); }						/* index to last input image, (optional) */
...
char *normalization)				/* optional tag for normalization */
{
	...
	fprintf(f,"$ws_detectorNumber		%d				// detector number used for this reconstruction\n",detNum);
...
{
	/* globals printed here:	in_header, imaging_parameters, image_set, user_preferences */
	double	X1=NAN, Y1, Z1, H1=NAN, F1=NAN;
	double	keV=NAN, value;
	long	scanNum;
...
int getParentPath(				/* make a duplicate of a file, but re-pack it.  This will reclaim lost space & it always overwites existing file.  Sends a system call */
char *path)
{
	char	cmdbuf[100];				/* ps command */
	char	out[2*FILENAME_MAX];		/* ps command result */
	size_t	len;
	char	*p, *pend;
	int		find;
	FILE	*pipe=NULL;
	...
	sprintf(cmdbuf,"ps -o command -p %d",pid);
	pipe = popen(cmdbuf,"r");
	wait(&stat_loc);
	len = fread(out,sizeof(char), 2*FILENAME_MAX, pipe);
	pclose(pipe);
	if (len<35) { path[0]='\0'; return 1; }

	p = strstr(out,"COMMAND");
	if (!p)  { path[0]='\0'; return 1; }
	p += 8;

	find = (*p=='\"') ? '\"' : ' ';
	pend = strchr(p,find);

	if (p>=pend) { path[0]='\0'; return 1; }	/* find end of program name */
	*pend = '\0';

	pend = strrchr(p,'/');						/* find slash before program and after path */
	if (p>=pend) { path[0]='\0'; return 1; }

	pend++;
	*pend = '\0';								/* leave on the trailing slash */
//	printf("outFINAL = '%s'\n",p);
	if (strlen(p)<1) { path[0]='\0'; return 1; }
	strncpy(path, p, FILENAME_MAX);
	return 0;
}
