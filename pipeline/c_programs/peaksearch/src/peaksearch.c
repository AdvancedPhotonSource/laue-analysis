...
char	*pgm)					/* name of this program */
{
	...
	fprintf(output,"$xDimDet		%u		// total number of un-binned pixels in detector along X\n",header->xDimDet);
	fprintf(output,"$yDimDet		%u		// total number of un-binned pixels in detector along Y\n",header->yDimDet);
	fprintf(output,"$startx			%u			// starting X of ROI (un-binned pixels)\n",header->startx-1);
	fprintf(output,"$endx			%u		// last X of ROI (un-binned pixels)\n",header->endx-1);
	fprintf(output,"$groupx			%u			// binning along X for the ROI (un-binned pixels)\n",header->groupx);
	fprintf(output,"$starty			%u			// starting Y of ROI (un-binned pixels)\n",header->starty-1);
	fprintf(output,"$endy			%u		// last Y of ROI (un-binned pixels)\n",header->endy-1);
	fprintf(output,"$groupy			%u			// binning along Y for the ROI (un-binned pixels)\n",header->groupy);
	...
	fprintf(stdout, "mask file"); // %d", exH->maskFile);
