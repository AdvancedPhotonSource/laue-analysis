...
long	l)
{
	...
	allowed = N>0 ? (Fr*Fr+Fi*Fi)/(N*N) > 0.01 : 0;
	return allowed;
}
...
double  *Fimag)
{
	...
	latticeCentering = p ? (int)(p-PFIRCA) : -1;
...
//...)									/* only one extra arg is allowed, that it Temperature (C) */
{
///	double	T=-500;						/* Temperature (C), Temperature is referenced to ROOM_TEMP == 22.5 */
...
//	va_start(ap,l);
//	printf("	ap = %p	*ap = %d\n",ap,*ap);
//	T = (*ap) ? va_arg(ap,double) : -500.;	/* get T if present, -500 causes the alphaT to be skipped */
...
struct crystalStructure *xtal)			/* lattice */
{
	...
	if (xtal->Ntype > 0) {
		Ntype = MIN(STRUCTURE_ATOMS_MAX,xtal->Ntype);
		type = xtal->atomType;
	}
	...
	realloc(atom,N*sizeof(*atom));		/* remove extra space (due to duplicate atom positions) */
	xtal->N = N;

#if (DEBUG>2)
	printf ("in setAtomXYZs found %ld atoms at locations:\n",N);
	for (i=0;i<N;i++) printf("xyz[%3d] = (%.3f   %.3f   %.3f)\n",i,atom[i].x,atom[i].y,atom[i].z);
#endif
...
struct crystalStructure *sourceLat)		/* source lattice */
{
	...
	destLat->equiv.N			= sourceLat->equiv.N;
	if (sourceLat->equiv.N > 0 && SG >= 1 && SG <= 230) {	/* copy the equivalent atome matricies */
		size_t	m,j, Neq;
...
struct crystalStructure *xtal)			/* lattice and all of the crystal structure values */
{
	...
	if (N>50) printf("     ... only showing the first 50\n");
...
struct crystalStructure *xtal)			/* lattice and all of the crystal structure values */
{
	...
	if (type>=Ntype) return -1;			/* an error */
